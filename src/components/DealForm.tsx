...
export default function DealForm({ mode, dealId, initialForm }: { mode: 'create' | 'edit'; dealId?: string; initialForm?: DealFormData }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'create') {
        ...
        await fetch('/api/ai/generate', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ dealId }) })
  ...
  const breadcrumbs = mode === 'edit'
    ? [{ label:'Dashboard', href:'/dashboard' }, { label: form.client_name || 'Deal', href: dealId ? `/deals/${dealId}` : undefined }, { label:'Edit' }]
    : [{ label:'Dashboard', href:'/dashboard' }, { label:'New Deal' }]
  const submitLabel = mode === 'create'
    ? (loading ? 'Calculating...' : 'Calculate & Greenlight Deal')
    : (submitPhase === 'regenerating' ? 'Regenerating documents...' : loading ? 'Saving...' : 'Save & Regenerate Documents')
  const helperText = mode === 'create'
    ? 'Instant results · AI documents generated after'
    : 'This recalculates profitability and regenerates all 4 AI documents'

  return (
    <>
      <Nav breadcrumbs={breadcrumbs} actions={<Link href={cancelHref} className="gl-btn gl-btn-ghost">Cancel</Link>}/>
