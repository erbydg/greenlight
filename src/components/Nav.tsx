'use client'
import { useState } from 'react'
import Link from 'next/link'
import { useRouter, usePathname } from 'next/navigation'
import { createSupabaseClient } from '@/lib/supabase-client'
import { useLocale } from '@/lib/i18n/LocaleProvider'
import LocaleToggle from '@/components/LocaleToggle'

interface Crumb { label: string; href?: string }

export default function Nav({ breadcrumbs, actions }: { breadcrumbs?: Crumb[]; actions?: React.ReactNode }) {
  const router = useRouter()
  const pathname = usePathname()
  const { d } = useLocale()
  const [menuOpen, setMenuOpen] = useState(false)

  const handleLogout = async () => {
    const supabase = createSupabaseClient()
    await supabase.auth.signOut()
    router.push('/login')
  }

  const navLinks = [
    { href: '/dashboard', label: d.nav.dashboard },
    { href: '/dashboard/completed', label: d.nav.completed },
    { href: '/settings', label: d.nav.settings },
  ]

  return (
    <nav className="gl-nav">
      <div className="gl-nav-row">
        <Link href="/dashboard" style={{ display:'flex', alignItems:'center', gap:9, textDecoration:'none', color:'var(--text)', flexShrink:0 }}>
          <div style={{ width:28, height:28, background:'var(--ink)', borderRadius:6, display:'grid', placeItems:'center', position:'relative', overflow:'hidden', flexShrink:0 }}>
            <svg width="12" height="12" viewBox="0 0 12 12" fill="none" style={{ position:'relative', zIndex:1 }}>
              <path d="M6 1L9 6H3L6 1Z" fill="white" opacity="0.9"/>
              <rect x="3.5" y="7.5" width="5" height="3" rx="0.8" fill="white" opacity="0.4"/>
            </svg>
            <span className="logo-blink" style={{ position:'absolute', width:7, height:7, background:'var(--green)', borderRadius:'50%', bottom:5, right:5, boxShadow:'0 0 5px var(--green)' }}/>
          </div>
          <span className="font-heading" style={{ fontSize:'1rem', fontWeight:600, letterSpacing:'-0.01em' }}>Greenlight</span>
        </Link>

        <div className="gl-nav-links">
          {navLinks.map(link => (
            <Link key={link.href} href={link.href} style={{
              padding:'5px 12px', borderRadius:6, fontSize:'0.78rem', fontWeight:500,
              textDecoration:'none', transition:'all 0.12s',
              color: pathname === link.href ? 'var(--text)' : 'var(--text-muted)',
              background: pathname === link.href ? 'var(--bg)' : 'transparent',
            }}>
              {link.label}
            </Link>
          ))}
        </div>

        {breadcrumbs && (
          <div className="gl-nav-crumbs">
            {breadcrumbs.map((c, i) => (
              <span key={i} style={{ display:'flex', alignItems:'center', gap:7 }}>
                {i > 0 && <span style={{ color:'var(--text-light)' }}>/</span>}
                {c.href
                  ? <Link href={c.href} style={{ color:'var(--text-muted)', textDecoration:'none' }}>{c.label}</Link>
                  : <span style={{ color:'var(--text)', fontWeight:500 }}>{c.label}</span>}
              </span>
            ))}
          </div>
        )}

        <div className="gl-nav-right">
          <span className="gl-badge gl-badge-low" style={{ whiteSpace:'nowrap' }}>
            <span className="gl-badge-dot"/>{d.nav.trialBadge}
          </span>
          <Link href="/settings#pricing" style={{ fontSize:'0.72rem', color:'var(--text-muted)', textDecoration:'underline', textUnderlineOffset:2, whiteSpace:'nowrap' }}>
            {d.nav.futurePricing}
          </Link>
          <LocaleToggle />
          {actions}
          <button onClick={handleLogout} className="gl-btn gl-btn-ghost" style={{ fontSize:'0.75rem', padding:'5px 12px' }}>
            {d.nav.signOut}
          </button>
        </div>

        <button
          className="gl-nav-hamburger"
          onClick={() => setMenuOpen(o => !o)}
          aria-label={d.nav.menuToggle}
          aria-expanded={menuOpen}
        >
          {menuOpen
            ? <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 6 6 18M6 6l12 12"/></svg>
            : <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 6h18M3 12h18M3 18h18"/></svg>}
        </button>
      </div>

      {menuOpen && (
        <div className="gl-nav-mobile-panel">
          {breadcrumbs && (
            <div className="gl-nav-crumbs" style={{ marginBottom: 4 }}>
              {breadcrumbs.map((c, i) => (
                <span key={i} style={{ display:'flex', alignItems:'center', gap:7 }}>
                  {i > 0 && <span style={{ color:'var(--text-light)' }}>/</span>}
                  {c.href
                    ? <Link href={c.href} onClick={() => setMenuOpen(false)} style={{ color:'var(--text-muted)', textDecoration:'none' }}>{c.label}</Link>
                    : <span style={{ color:'var(--text)', fontWeight:500 }}>{c.label}</span>}
                </span>
              ))}
            </div>
          )}

          {navLinks.map(link => (
            <Link key={link.href} href={link.href} onClick={() => setMenuOpen(false)} className="gl-nav-mobile-link" style={{
              color: pathname === link.href ? 'var(--text)' : 'var(--text-muted)',
              background: pathname === link.href ? 'var(--bg)' : 'transparent',
            }}>
              {link.label}
            </Link>
          ))}

          <div className="gl-nav-mobile-divider" />

          <span className="gl-badge gl-badge-low" style={{ alignSelf:'flex-start' }}>
            <span className="gl-badge-dot"/>{d.nav.trialBadge}
          </span>
          <Link href="/settings#pricing" onClick={() => setMenuOpen(false)} style={{ fontSize:'0.78rem', color:'var(--text-muted)', textDecoration:'underline', textUnderlineOffset:2 }}>
            {d.nav.futurePricing}
          </Link>

          {actions && <div onClick={() => setMenuOpen(false)} style={{ display:'flex', flexWrap:'wrap', gap:8 }}>{actions}</div>}

          <div className="gl-nav-mobile-divider" />

          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:10 }}>
            <LocaleToggle />
            <button onClick={handleLogout} className="gl-btn gl-btn-ghost" style={{ fontSize:'0.8rem', padding:'10px 16px' }}>
              {d.nav.signOut}
            </button>
          </div>
        </div>
      )}
    </nav>
  )
}
